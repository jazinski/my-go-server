...
func pushHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	// Debug: Log to stderr what we receive
	fmt.Fprintf(os.Stderr, "DEBUG: pushHandler called with request: %+v\n", request)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "DEBUG: RequireString error: %v\n", err)
		return mcp.NewToolResultError(err.Error()), nil
	}
	
	// Additional check for empty message
	if strings.TrimSpace(message) == "" {
		fmt.Fprintf(os.Stderr, "DEBUG: Empty message detected\n")
		return mcp.NewToolResultError("message parameter cannot be empty"), nil
	}
	
	fmt.Fprintf(os.Stderr, "DEBUG: Message received: '%s'\n", message)
