...
func pyToolHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Now execute the docker command
	execCmd := exec.Command("docker", cmdArgs...)
	out, err := execCmd.Output()
	
	if err != nil {
		if exitErr, ok := err.(*exec.ExitError); ok {
			return mcp.NewToolResultText(fmt.Sprintf("Error executing Python code: %s", exitErr.ExitCode(), string(exitErr.Stderr))), nil
		}
		return mcp.NewToolResultError(fmt.Sprintf("Failed to execute Python code: %v", err)), nil
	}


	// For now, return a placeholder since executePythonCode is not implemented
	return mcp.NewToolResultText(fmt.Sprintf("Python code execution not yet implemented. Code: %s, Modules: %v", code, modules)), nil
}	