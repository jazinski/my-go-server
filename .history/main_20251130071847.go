...
func main() {
	...
	pushTool := mcp.NewTool("send_push_notification",
		...
		mcp.WithString("user_key", mcp.Required(), mcp.Description("Pushover user key")),
...
func pushHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	userKey, err := request.RequireString("user_key")
	if err != nil {
		return mcp.NewToolResultError(err.Error()), nil
	}
	...
	cmd := exec.Command("curl", "-s",
		"--form-string", fmt.Sprintf("token=%s", "YOUR_APP_TOKEN_HERE"),
